...
//                    opts.format = OutputFormat.XML;
    ...
    public void run() throws Exception {
        ...
        ActionsIoUtils.ActionSerializer serializer = opts.format.getSerializer(
                diff.src, diff.dst, diff.editScript, diff.mappings);
        if (opts.output == null)
...
//        XML {
//            @Override
//            ActionsIoUtils.ActionSerializer getSerializer(TreeContext sctx,
//                                                   EditScript actions, MappingStore mappings)
//                    throws IOException {
...
//        },
        JSON {
            ...
            ActionsIoUtils.ActionSerializer getSerializer(
                    TreeContext oldCtx, TreeContext newCtx, EditScript actions, MappingStore mappings)
                    throws IOException {
                return ActionsIoUtils.toJson(oldCtx , newCtx , actions, mappings);
            }
        };
...
//                                                               MappingStore mappings) throws IOException;
        abstract ActionsIoUtils.ActionSerializer getSerializer(TreeContext sctx, TreeContext newCtx, EditScript actions,
                                                               MappingStore mappings) throws IOException;
    }
}
