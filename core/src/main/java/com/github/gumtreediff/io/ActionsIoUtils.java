...
public final class ActionsIoUtils {
    ...
    public static ActionSerializer toText(TreeContext sctx, TreeContext newCtx,EditScript actions,
                                          MappingStore mappings) throws IOException {
        return new ActionSerializer(sctx, newCtx,  mappings, actions) {

            @Override
            protected ActionFormatter newFormatter(
                TreeContext ctx, TreeContext newCtx, Writer writer) throws Exception {
                return new TextFormatter(ctx, writer);
            }
        };
    }

    public static ActionSerializer toXml(TreeContext sctx, TreeContext newCtx, EditScript actions,
                                         MappingStore mappings) throws IOException {
        return new ActionSerializer(sctx,  newCtx, mappings, actions) {
    ...
    public static ActionSerializer toJson(TreeContext sctx, TreeContext newCtx, EditScript actions,
                                              MappingStore mappings) throws IOException {
        return new ActionSerializer(sctx, newCtx , mappings, actions) {

            @Override
            protected ActionFormatter newFormatter(
                TreeContext oldCtx, TreeContext newCtx, Writer writer) throws Exception {
                return new JsonFormatter(oldCtx, newCtx, writer);
            }
        };
    }
    ...
    public abstract static class ActionSerializer extends AbstractSerializer {
        ...
        final TreeContext newCtx;
        final MappingStore mappings;
        final EditScript actions;

        ActionSerializer(TreeContext context, TreeContext newContext, MappingStore mappings, EditScript actions) {
        ...
        protected abstract ActionFormatter newFormatter(TreeContext ctx, TreeContext newCtx, Writer writer) throws Exception;

        @Override
        public void writeTo(Writer writer) throws Exception {
            ActionFormatter fmt = newFormatter(context, newCtx, writer);
            // Start the output
            fmt.startOutput();
...
//            writer.writeEndElement();
    ...
    static class TextFormatter implements ActionFormatter {
        ...
        public void insertRoot(Insert action, Tree node) throws Exception {
            ...
            write(String.valueOf(node.getPos()));
        }
        ...
        public void moveAction(Move action, Tree src, Tree dst, int position) throws Exception {
            write(action.toString());
            write(String.valueOf(dst.getPos()));
        }
    ...
    static class JsonFormatter implements ActionFormatter {
        ...
        private final TreeContext oldCtx;
        private final TreeContext newCtx;

        JsonFormatter(TreeContext oldCtx, TreeContext newCtx, Writer writer) {
        ...
        public void insertAction(Insert action, Tree node, Tree parent, int index) throws IOException {
            start(action, node);
//            writer.name("offset").value(node.getPos());
...
//            writer.name("offset").value(node.getPos());
        ...
        public void moveAction(Move action, Tree src, Tree dst, int index) throws IOException {
            start(action, dst);
//            writer.name("offset").value(dst.getPos());
...
//            writer.name("offset").value(node.getPos());
        ...
        private void start(Action action, Tree src) throws IOException {
            ...
            writer.name("stringRepresentation").value(src.toTreeString());
            writer.name("treelength").value(src.getLength());
            writer.name("StartingOffset").value(src.getPos());
            writer.name("EndingOffset").value(src.getEndPos());
            writer.name("startRowNumber").value(newCtx.getLineReader().positionFor(src.getPos())[0]);
            writer.name("startColumnNumber").value(newCtx.getLineReader().positionFor(src.getPos())[1]);
            writer.name("endRowNumber").value(newCtx.getLineReader().positionFor(src.getEndPos())[0]);
            writer.name("endColumnNumber").value(newCtx.getLineReader().positionFor(src.getEndPos())[1]);
            writer.name("oldlines").value(oldCtx.getLineReader().getLines().toString());
            writer.name("newlines").value(newCtx.getLineReader().getLines().toString());
//            var finishChar = src.getLength();
//            writer.name("endRowNumber").value(ctx.getLineReader().positionFor(
//                    src.getPos() + src.getLength()
//            )[0]);
//            writer.name("endColumnNumber").value(ctx.getLineReader().positionFor(src.getPos() + src.getLength())[1]);
        }
